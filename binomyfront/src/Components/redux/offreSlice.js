...
export const fetchOffres = createAsyncThunk ("offre/all",async()=> {try {
    let result = axios.get("http://localhost:5000/offre/all")
    return result;
} catch (error) {
    console.log(error);
}})

export const fetchOffresByOwner = createAsyncThunk("offre/byOwner", async (id, { rejectWithValue }) => {
    try {
      const result = await axios.get(`http://localhost:5000/offre/${id}`);
...
export const addOffre = createAsyncThunk("offre/add", async (formData, { rejectWithValue }) => {
    try {
      const result = await axios.post("http://localhost:5000/offre/add", formData);
  ...
  export const deleteoffre = createAsyncThunk("offre/delete", async (id, { rejectWithValue }) => {
    try {
      await axios.delete(`http://localhost:5000/offre/${id}`);
...
export const editoffre = createAsyncThunk ("offre/update",async({id,editedoffre})=> {try {
    let result = axios.put (`http://localhost:5000/offre/${id}`,editedoffre)
    return result;
} catch (error) {
    console.log(error);
}})
